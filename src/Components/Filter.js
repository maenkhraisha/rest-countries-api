...
const Filter = ({ filterContinent, filterCoutry }) => {
  ...
  return (
    <div className="bg-zinc-100 py-10 px-16 flex flex-col gap-4
                   md:flex-row md:justify-between dark:text-white">
      <div>
        <BsSearch className="absolute mt-3 ml-3" />
        <input
          className="flex-1 py-2 pl-11 w-full focus:outline-none dark:bg-darkLigth "
          type="text"
      ...
      <select
        className="py-2 px-4 max-w-xs focus:outline-none dark:bg-darkLigth"
        name="category"
        value={continent}
